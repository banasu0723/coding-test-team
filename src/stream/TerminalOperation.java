...
public class TerminalOperation {
    ...
    public static void main(String[] args) {
        ...
        //IntFunction<R> : 원시형 int → R : Stream.toArray()에서 쓰인다.
        System.out.println("3. toArray - 배열로 변환");
        Integer[] arr = numbers.stream()
                .filter(n -> n % 2 == 0)
                .toArray(Integer[]::new); // value -> new Integer[value];
        ...
        int[] array = numbers.stream()
                .filter(n -> n % 2 == 0)
                .mapToInt(e -> e)
                .toArray();//IntStream 의 toArray() != Stream 의 toArray(IntFunction<R> function)
        ...
        // 초기값 100부터 시작해서 하나씩 더해간다.


        //Stream<T> 의 min, max 는 무엇으로 대/소 비교를 할지 를 넣어주어야한다.기준을 정해줘야해 명시적으로
        //But IntStream 기본형 스트림은 안정해줘도 된다. 이건 toArray()랑 같은 이유이다.
        ...
        System.out.println("최솟값: " + max.get());
        System.out.println();
